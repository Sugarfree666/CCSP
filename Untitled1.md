...
输入问题 $Q$：我想购买一台苹果品牌笔记本电脑，内存至少为 16GB，重量不高于1.5kg，价格不高于8000元。
...
2. 通过prompt将根节点分解为4个子约束任务，使用**生成思维**生成四个思考结点
   ...
   3. $V_3$ 查找重量低于1.5kg的笔记本电脑（Filter）
...
3. 使用**聚合思维**将思考$V_{1},V_{2}$和$V_{3},V_{4}$分别两两聚合。得到的聚合结果利用**生成思维** 得到$V_{12}$和 $V_{34}$ 作为下一思考结点。
   ...
   > 对于思考节点中的实体，根据社区检测找到在KG中联系紧密的实体进行聚合，不再盲目的聚合。

   - 将之前的查询集合进行对应的聚合得到 $A_m$与 $A_n$。LLM会根据这个聚合后的集合进行查找。
...
- 比如：$V_{A}$ (轻薄) 和 $V_{B}$ (高性能) 聚合后结果为空，不仅仅让 LLM 瞎猜“放宽条件”，而是去 KG 中寻找连接 $V_{A}$ 和 $V_{B}$ 的 **最短路径** 或 **桥接节点**。
