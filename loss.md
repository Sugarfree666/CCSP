...
**Focal Loss**，作用是降低容易样本的损失权重，让模型把训练重点放在难样本上。是在交叉熵损失的基础上增加了一个调制因子 $\mathbf{(1-p_t)^\gamma}$，$1-\mathbf{p_t}$是模型预测错误的概率，$\mathbf{\gamma}$是一个可调节的参数。对于易分类样本损失，$(1-p_t)^\gamma \approx 0$,损失贡献大幅降低。相反对于难分类样本，损失贡献保持不变。
$$
FL(p_t) = -\mathbf{(1-p_t)^\gamma} \log(p_t)
$$
