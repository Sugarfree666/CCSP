...
> 我想找一种高血压药物，要求：1. 不含‘磺胺’成分（因为我过敏）；2. 不会与我正在服用的‘阿司匹林’产生拮抗作用；3. 属于医保乙类。

这种问题一般包含以下几种约束：

- **否定约束**：不含‘磺胺’成分。这在传统QA中很难处理，因为模型习惯于寻找“存在什么”，而不是“不存在什么”。
- **关系约束**：不会与‘阿司匹林’产生拮抗作用。这需要理解药物之间的“拮抗”关系，并进行多步推理（找到候选药物 -> 检查其与阿司匹林的关系）。
- **属性约束**：属于医保乙类。这是一个简单的属性过滤。

可能的挑战：

1. 对于否定约束，LLM在否定性约束判定问题存在困难，KG存储的是“存在”的知识，但“不含磺胺”是一个**否定性知识**。这种查询在使用大型KG中效率较低。（可能的方法是混合检索）
2. 
